...
# path_to_hesse = os.path.join(main_eftDir, "eft_hesse", 'robustHessefirstStep.root')
...
for i, current_poi in enumerate(poi_list):
    ...
    # with uproot.open(os.path.join(combineLL_eftDir, f"higgsCombinefirstStep_{current_poi.replace(f'r_{variable}', eft_variable)}.MultiDimFit.mH125.38.root")) as file:
...
# def MuToWC(mu, current_poi, eft_variable):
...
#     current_poi_eft2obsConvention = poi_to_eft2obs_namingConvention[current_poi]

#     # Extract the A's and B's from production and decay where applicable
#     A_prod = production.get(current_poi_eft2obsConvention, {}).get(f"A_{eft_variable}", 0)
#     B_prod = production.get(current_poi_eft2obsConvention, {}).get(f"B_{eft_variable}", 0)

#     A_decay = decay.get("gamgam", {}).get(f"A_{eft_variable}", 0)
#     B_decay = decay.get("gamgam", {}).get(f"B_{eft_variable}", 0)

#     A_tot = decay.get("tot", {}).get(f"A_{eft_variable}", 0)
#     B_tot = decay.get("tot", {}).get(f"B_{eft_variable}", 0)
...
def WCToMu(wc, _current_poi, _eft_variable):
    ...
    # print(f"WCToMu: {current_poi_eft2obsConvention}, {_eft_variable}, wc: {wc}, A_prod: {A_prod}, B_prod: {B_prod}, A_decay: {A_decay}, B_decay: {B_decay}, A_tot: {A_tot}, B_tot: {B_tot}")
...
def produce_and_minimize_chi(pois_, poi_list_, first_order=False):
    ...
    if first_order:
        # eft_poi_list = [poi.replace(f"r_{variable}", eft_variable) for poi in poi_list_]
