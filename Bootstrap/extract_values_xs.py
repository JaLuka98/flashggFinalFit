...
def chi_vector(mu_exp, mu_obs, _a, _b, _c):
    ...
    if _c == 0:
        ...
        chi_exp = (np.sqrt(_b**2 - 4*(_a-mu_exp)*_c) - _b) / (2*_c)
        chi_obs = (np.sqrt(_b**2 - 4*(_a-mu_obs)*_c) - _b) / (2*_c)
    
    chi_diff = (chi_obs - chi_exp) # Chi_Obs is very small compared to chi_exp
    return chi_diff

def chi(mu, pois_, poi_list_, rho_, abc_values=None, first_order=False, bf_combine_=None):
    chi_vector_ = []
    ...
    if first_order:
        ...
        for i, current_poi in enumerate(poi_list_):
            chi_vector_.append([(bf_combine_[i] - mu[i])])
        ...
        for i, current_poi in enumerate(poi_list_):
            ...
            chi_vector_.append([chi_vector(mu[i], mean, a, b, c, current_poi)])
...
def produce_and_minimize_chi_crossingMethod(pois_, poi_list_, combineLL_dir_, inclusive_=False):
...
def produce_and_minimize_chi(pois_, poi_list_, subfolder_, path_to_hesse_, first_order=False, bf_combine_=None, inclusive_=False, plot_r_distribution=True):
    ...
    if first_order:
        ...
        res = minimize(chi, x0, args=(pois_, poi_list_, rho, abc_values, first_order, bf_combine_))
    ...
    for i, current_poi in enumerate(poi_list_):
        ...
        # Create a grid of points
        x0_range = np.linspace(-4, 4, 200)  # Adjust range as needed
        ...
        for x0 in x0_range:
            for j in range(len(poi_list_)):
                ...
                if j == i:
                    optimal_values_copy[j] = x0
            if first_order:
                chi_x0_scan.append(float(chi(optimal_values_copy, pois_, poi_list_, rho, abc_values, first_order=True, bf_combine_=bf_combine_)))
            else:
...
def produce_LLPlots(pois_, poi_list_, combineLL_dir_, path_to_hesse_, folder="", subfolder_="", print_first_order=False, bf_combine_=None, with_crossingMethod=False, inclusive_=False, plot_r_distribution=True):

    x0_ranges, chi_x0_scans, optimal_values = produce_and_minimize_chi(pois_, poi_list_, subfolder_, path_to_hesse_, bf_combine_=bf_combine_, inclusive_=inclusive_, plot_r_distribution=plot_r_distribution)

    if print_first_order:
        x0_ranges_fo, chi_x0_scans_fo, optimal_values_fo = produce_and_minimize_chi(pois_, poi_list_, subfolder_, path_to_hesse_, first_order=True, bf_combine_=bf_combine_, inclusive_=inclusive_, plot_r_distribution=plot_r_distribution)

    if with_crossingMethod:
        x0_ranges_cm, chi_x0_scans_cm, optimal_values_cm = produce_and_minimize_chi_crossingMethod(pois_, poi_list_, combineLL_dir_, inclusive_=inclusive_)
    ...
    for i, current_poi in enumerate(poi_list_):
        ...
        if inclusive_:
            combine_file_path = os.path.join(combineLL_dir_, "scans", f"scan_{variable}.root")
        else:
            combine_file_path = os.path.join(combineLL_dir_, "scans", f"scan_{current_poi}.root")
        ...
        if inclusive_:
            ax1.set_xlabel(f"{translation[f'{variable}']}")
        else:
            ax1.set_xlabel(f"{translation[f'{current_poi}']}")
        ...
        plt.tight_layout()
        if inclusive_:
            plt.savefig(os.path.join(folder, f"chi_scan_{variable}.pdf"))
            plt.savefig(os.path.join(folder, f"chi_scan_{variable}.png"))
        else:
...
for variable in variables:
    ...
    main_dir = os.path.join(sample_dir, variable+"_onlyPreEE_RandomizeAux", "Combine", f"runFits_{variable}")
