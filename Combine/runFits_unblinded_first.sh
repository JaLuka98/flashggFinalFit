...
cp "./Datacard_${differential_variable}.root" "$folder_name/unblinded/Datacard_${differential_variable}.root"

cd "runFits_${differential_variable}/unblinded"

for param in ${paramStrNoOne//\,/\ }
do
  combineTool.py -M MultiDimFit Datacard_${differential_variable}.root --freezeParameters MH -m 125.38 -n DataPostFitScanFit_${param} \
  --cminDefaultMinimizerStrategy=0 --algo grid --points 30 \
  --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants \
  --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=2 -P $param --saveFitResult \
  --job-mode condor --task-name first_DataScan_${param} --sub-opts='+JobFlavour = "workday"' \
  --floatOtherPOIs 1 --saveWorkspace --saveSpecifiedIndex ${pdfIndeces}
...
./multi_submit.sh $differential_variable "unblinded" "condor_first"

echo "Unblinded fits (first step) for ${differential_variable} sent to condor."