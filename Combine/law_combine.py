...
class PrepareTheDirectory(Task, SlurmWorkflow, law.LocalWorkflow):#(law.Task): #(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def requires(self):
        ...
        tasks = [MakeDatacard(output_dir=output_dir, variable=self.variable, year=self.year, bootstrap_flag=self.bootstrap_flag, number_of_bootstraps=self.number_of_bootstraps, version="v1", batch_flavor=self.batch_flavor)]
    ...
    def run(self):
        ...
        if convert_boolean_string(self.bootstrap_flag) == True:
            ...
            if self.batch_flavor == "slurm/psi":
                # execute_command([f'xrdcp -rf root://t3dcachedb.psi.ch:1094/{signal_src_path} root://t3dcachedb.psi.ch:1094/{signal_dst_path}'], shell=True)
                # execute_command([f'xrdfs root://t3dcachedb.psi.ch:1094 ls {signal_dst_path}'], shell=True)
                manually_copy_t3(signal_src_path, signal_dst_path)
            else:
                shutil.copytree(signal_src_path, signal_dst_path, dirs_exist_ok=True)
...
class RunText2Workspace(Task, SlurmWorkflow, law.LocalWorkflow): #(law.Task): #(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def requires(self):
        ...
        tasks = [PrepareTheDirectory(output_dir=output_dir, variable=self.variable, year=self.year, bootstrap_flag=self.bootstrap_flag, number_of_bootstraps=self.number_of_bootstraps, version="v1", batch_flavor=self.batch_flavor)]
    ...
    def run(self):      
        ...
        if convert_boolean_string(self.bootstrap_flag) == True:
            ...
            if self.batch_flavor == "slurm/psi":
                ...
                slurm_copy_command = [
                    'xrdcp', '-rf',
                    'root://t3dcachedb.psi.ch:1094//'+ f'{output_dir}/Combine/Datacards/{datacard_name}.txt',
...
class AsimovCovCorr(Task, SlurmWorkflow, HTCondorWorkflow, law.LocalWorkflow): #(law.Task): #(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def run(self):
        ...
        if self.batch_flavor == "slurm/psi":
            ...
            # os.chdir(os.path.join(os.environ["TARGET_PATH"], 'Combine', fitFolderName, 'hesse', 'Plots'))
        else:
            execute_command([f'mkdir -p {output_dir}/Combine/{fitFolderName}/hesse/Plots'], shell=True)
            output_dir = os.path.join(output_dir, 'Combine', fitFolderName, 'hesse', 'Plots')
            # os.chdir(os.path.join(output_dir, 'Combine', fitFolderName, 'hesse', 'Plots'))
...
class ToyFitCategoryFirstStep(Task, SlurmWorkflow, HTCondorWorkflow, law.LocalWorkflow): #(law.Task): #(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def create_branch_map(self):
        ...
        branch_map = {
            i * int(self.number_of_toys) + j: (current_cat, toy_index)
    ...
    def output(self):
        current_cat, toy_index = self.branch_data
        ...
        if self.variable != '':
            output += [os.path.join(output_dir, 'Combine', fitFolderName, 'toyFit', f'toy_{toy_index}', f'higgsCombinefirstStep_{current_cat}.MultiDimFit.mH125.38.{seed}.root')]
            output += [os.path.join(output_dir, 'Combine', fitFolderName, 'toyFit', f'toy_{toy_index}', f'multidimfitfirstStep_{current_cat}.root')]
...
class ToysFitSystSingle(Task, SlurmWorkflow, HTCondorWorkflow, law.LocalWorkflow): #(law.Task): #(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def requires(self):
        ...
        # tasks = [ToyFitCategoryFirstStep(output_dir=output_dir, variable=self.variable, year=self.year, number_of_toys=self.number_of_toys, version='v1', batch_flavor=self.batch_flavor, seed=self.seed)]
        tasks = [RunText2Workspace(output_dir=output_dir, variable=self.variable, year=self.year, bootstrap_flag=False, number_of_bootstraps=1000, version='v1', batch_flavor=self.batch_flavor)]
    ...
    def output(self):
        ...
        output = [os.path.join(output_dir, 'Combine', fitFolderName, 'toyFit', f'toy_{toy_index}')]
        output += [os.path.join(output_dir, 'Combine', fitFolderName, 'toyFit', f'toy_{toy_index}', f'higgsCombineToyBestFit_{current_cat}.MultiDimFit.mH125.38.{seed}.root')]
        output += [os.path.join(output_dir, 'Combine', fitFolderName, 'toyFit', f'toy_{toy_index}', f'multidimfitToyBestFit_{current_cat}.root')]
    ...
    def run(self):
        ...
        toy_output = os.path.join(output_dir, 'Combine', fitFolderName, 'toyFit', f'toy_{toy_index}')
            
        def check_pdf_idx(toy_output_, param_, seed_):
            # Run the ROOT command
            command = f'root -l -q \'{os.environ["ANALYSIS_PATH"]}/Combine/checkPdfIdx.C("{toy_output_}/higgsCombinefirstStep_{param_}.MultiDimFit.mH125.38.{seed_}.root")\''
...
class UnblindedFitSystSingle(Task, SlurmWorkflow, HTCondorWorkflow, law.LocalWorkflow): #(law.Task): #(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def run(self):
        ...
        if convert_boolean_string(self.bootstrap_flag) == True:
            ...
            if self.batch_flavor == "slurm/psi":
                # Have to use /scratch/batch_username/ for slurm/psi
                execute_command([f'xrdfs root://t3dcachedb.psi.ch:1094/ mkdir -p {output_dir}/Combine/{fitFolderName}/dataFit/bootstrap_{bootstrap_index}'], shell=True)
