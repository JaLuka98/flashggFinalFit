...
# Submit the 2024 PTH differential kappa_u expected NLL scan to Condor.
#
# Build the workspace first with:
#   bash light_quarks/light_quarks_t2w_differential_uuH.sh
#
# Usage:
#   bash light_quarks/light_quarks_run_limits_differential_uuH_condor.sh
...
WORKSPACE=${PTH_OUTPUT}/Combine/LightQuarks_kappau_Datacard_PTH_2024.root
OUTPUT_DIR=${PTH_OUTPUT}/LightQuarks_limits_kappau_PTH_condor
SCAN_RANGE="kappa_u=-3,3"
SCAN_POINTS=21
SPLIT_POINTS=1
MASS=125.38
JOB_FLAVOUR=workday
TASK_NAME=LightQuarks_2024_PTH_kappau_scan
COMBINE_NAME=LightQuarks_2024_PTH_kappau_expected
MERGED_OUTPUT=merged_LightQuarks_2024_PTH_kappau_expected.root

if [ ! -f "${WORKSPACE}" ]; then
    echo "ERROR: workspace not found:"
    echo "  ${WORKSPACE}"
    echo "Build it first with:"
    echo "  bash light_quarks/light_quarks_t2w_differential_uuH.sh"
    exit 1
fi
...
echo ""
echo "Submitting split uuH grid scan to Condor..."
