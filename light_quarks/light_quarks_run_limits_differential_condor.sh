...
# Submit the 2024 PTH differential kappa_q expected NLL scan to Condor,
# splitting the grid so each Condor job runs one scan point.
#
# Build the workspace first with:
#   bash light_quarks/light_quarks_t2w_differential.sh
#
# Usage:
#   bash light_quarks/light_quarks_run_limits_differential_condor.sh
...
OUTPUT_DIR=${PTH_OUTPUT}/LightQuarks_limits_kappaq_PTH_condor
SCAN_RANGE="kappa_q=-3,3"
SCAN_POINTS=21
SPLIT_POINTS=1
MASS=125.38
JOB_FLAVOUR=workday
TASK_NAME=LightQuarks_2024_PTH_kappaq_scan
COMBINE_NAME=LightQuarks_2024_PTH_expected
MERGED_OUTPUT=merged_LightQuarks_2024_PTH_expected.root

if [ ! -f "${WORKSPACE}" ]; then
    echo "ERROR: workspace not found:"
    echo "  ${WORKSPACE}"
    echo "Build it first with:"
    echo "  bash light_quarks/light_quarks_t2w_differential.sh"
    exit 1
fi
...
echo ""
echo "Submitting split grid scan to Condor..."
...
echo ""
echo "Then plot with:"
echo "  python3 ${FLASHGG_DIR}/light_quarks/plot_kappaq_scan.py --input ${OUTPUT_DIR}/${MERGED_OUTPUT} --output-dir ${OUTPUT_DIR} --title 'Expected limit: light quark Yukawa (kappa_q) - 2024 PTH Condor'"
set +e
