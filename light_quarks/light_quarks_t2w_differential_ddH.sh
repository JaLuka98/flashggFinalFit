...
#
# Physics model: light_quarks.py::light_quarks_differential_ddH
# POI: kappa_d
# BSM/SM production term: read from SM JSON and ddH CSV in light_quarks/
# Usage: bash light_quarks/light_quarks_t2w_differential_ddH.sh
...
OUTPUT=${PTH_OUTPUT}/Combine/LightQuarks_kappad_Datacard_PTH_2024.root
...
echo "Physics model:             light_quarks_differential_ddH"

text2workspace.py ${DATACARD} \
    -o ${OUTPUT} \
    -m 125.38 \
    higgsMassRange=122,128 \
    -P light_quarks:light_quarks_differential_ddH
