...
# Submit the 2024 PTH differential shape-only kappa_d expected NLL scan to Condor.
#
# Build the workspace first with:
#   bash light_quarks/light_quarks_t2w_differential_ddH_shape_only.sh
#
# Usage:
#   bash light_quarks/light_quarks_run_limits_differential_ddH_shape_only_condor.sh
...
WORKSPACE=${PTH_OUTPUT}/Combine/LightQuarks_kappad_shape_only_Datacard_PTH_2024.root
OUTPUT_DIR=${PTH_OUTPUT}/LightQuarks_limits_kappad_PTH_shape_only_condor
SCAN_RANGE="kappa_d=-3,3:BR_hgg=0,10"
SCAN_POINTS=21
SPLIT_POINTS=1
MASS=125.38
JOB_FLAVOUR=workday
TASK_NAME=LightQuarks_2024_PTH_shape_only_kappad_scan
COMBINE_NAME=LightQuarks_2024_PTH_shape_only_kappad_expected
MERGED_OUTPUT=merged_LightQuarks_2024_PTH_shape_only_kappad_expected.root

if [ ! -f "${WORKSPACE}" ]; then
    echo "ERROR: workspace not found:"
    echo "  ${WORKSPACE}"
    echo "Build it first with:"
    echo "  bash light_quarks/light_quarks_t2w_differential_ddH_shape_only.sh"
    exit 1
fi
...
echo ""
echo "Submitting split ddH shape-only grid scan to Condor..."
