...
#
# Physics model: light_quarks.py::light_quarks_differential_uuH_shape_only
# POI: kappa_u
# Shape-only normalization is profiled with one free common BR_hgg factor.
# Usage: bash light_quarks/light_quarks_t2w_differential_uuH_shape_only.sh
...
SHAPE_ONLY_DATACARD=${PTH_OUTPUT}/Combine/Datacard_PTH_2024_uuH_shape_only.txt
OUTPUT=${PTH_OUTPUT}/Combine/LightQuarks_kappau_shape_only_Datacard_PTH_2024.root
...
echo "Physics model:             light_quarks_differential_uuH_shape_only"
...
if ! grep -q "^BR_hgg[[:space:]]\\+flatParam" ${SHAPE_ONLY_DATACARD}; then
    {
        echo ""
        echo "# Free common signal-rate factor profiled in the shape-only uuH light-quark scan"
...
text2workspace.py ${SHAPE_ONLY_DATACARD} \
    ...
    -P light_quarks:light_quarks_differential_uuH_shape_only
