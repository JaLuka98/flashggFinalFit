...
# Merge and plot the 2022+2023+2024 PTH ssH differential shape-only scan.
...
for POINT in ${POINTS}; do
    ...
    if ! python3 -c "import sys, uproot; f=uproot.open(sys.argv[1]); t=f['limit']; a=t.arrays(['kappa_q','quantileExpected'], library='np'); assert len(a['kappa_q'][a['quantileExpected'] > -0.5]) > 0" "${FILE}" >/dev/null 2>&1; then
