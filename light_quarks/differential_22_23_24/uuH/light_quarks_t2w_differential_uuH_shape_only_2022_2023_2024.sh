...
# Build the 2022+2023+2024 PTH uuH differential shape-only workspace.
...
SHAPE_ONLY_DATACARD=${PTH_OUTPUT}/Combine/Datacard_PTH_2022_2023_2024_uuH_shape_only.txt
OUTPUT=${PTH_OUTPUT}/Combine/LightQuarks_kappau_uuH_shape_only_Datacard_PTH_2022_2023_2024.root
...
if ! grep -q "^BR_hgg[[:space:]]\\+flatParam" ${SHAPE_ONLY_DATACARD}; then
    echo "" >> ${SHAPE_ONLY_DATACARD}
    echo "BR_hgg flatParam" >> ${SHAPE_ONLY_DATACARD}
fi

echo "Building 2022+2023+2024 PTH uuH shape-only workspace"
echo "Datacard:  ${SHAPE_ONLY_DATACARD}"
echo "Output:    ${OUTPUT}"
