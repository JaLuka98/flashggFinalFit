...
# Submit the 2024 PTH differential shape-only kappa_q expected NLL scan to
# Condor, splitting the grid so each Condor job runs one scan point.
#
# Build the workspace first with:
#   bash light_quarks/light_quarks_t2w_differential_shape_only.sh
#
# Usage:
#   bash light_quarks/light_quarks_run_limits_differential_shape_only_condor.sh
...
OUTPUT_DIR=${PTH_OUTPUT}/LightQuarks_limits_kappaq_PTH_shape_only_condor
SCAN_RANGE="kappa_q=-3,3:BR_hgg=0,10"
SCAN_POINTS=21
SPLIT_POINTS=1
MASS=125.38
JOB_FLAVOUR=workday
TASK_NAME=LightQuarks_2024_PTH_shape_only_kappaq_scan
COMBINE_NAME=LightQuarks_2024_PTH_shape_only_expected
MERGED_OUTPUT=merged_LightQuarks_2024_PTH_shape_only_expected.root
...
echo ""
echo "Submitting split shape-only grid scan to Condor..."
...
echo ""
echo "Then plot with:"
echo "  python3 ${FLASHGG_DIR}/light_quarks/plot_kappaq_scan.py --input ${OUTPUT_DIR}/${MERGED_OUTPUT} --output-dir ${OUTPUT_DIR}"
set +e
