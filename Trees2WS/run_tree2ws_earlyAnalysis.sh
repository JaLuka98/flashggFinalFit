...
for modeString in "${productionModes[@]}"; do
    ...
    for era in "${eras[@]}"; do
        ...
        for mass in "${inputMasses[@]}"; do
            python trees2ws.py --inputMass "$mass" --productionMode "$mode" --year 2022"$era" --doSystematics --doDiffSplitting --inputConfig config_2022_PTH.py --inputTreeFile "$path_to_root_files"/"$process"_M-"$mass"_"$era"/*.root --outputWSDir "$outputDir"
        done
    done
done
